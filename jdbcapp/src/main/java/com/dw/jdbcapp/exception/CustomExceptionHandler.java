...
// 이 클래스의 역할은 개발자가 만든 사용자정읭{외들을 모두 직접 응답처리 하는 것
// 응답 메시지의 형태와 내용을 모두 결정할 수 있어서 편리함
// 백엔드 와 프론트엔드 개발자 사이에 협의된 형태로 전달하는 것이 일반적임!!


@Order(Ordered.HIGHEST_PRECEDENCE) // 중복된 예외처리의 우선순위로 결정
@RestControllerAdvice // 컨트롤러,서비스,레포지토리의 예외발생을 모두 이 클래스가 담당하도록 하는 것
public class CustomExceptionHandler {
    @ExceptionHandler(InvalidRequestException.class)
    // () 안에 선언한 예외클래스를 핸들링하는 메서드라는 의미
    // 자바에서는 클래스 이름만 필요한 경우에는 반드시 클래스명.class 를 사용해야함
